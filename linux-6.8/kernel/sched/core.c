...
change:
	...
	if (!(attr->sched_flags & SCHED_FLAG_KEEP_PARAMS)) {
		...
		//NEW CUSTOMIZED POLICY IMPLEMENTATION
		if (attr->sched_policy == SCHED_LOTTERY) {
			int tickets = attr->sched_priority - LOTTERY_PRIO_BASE;
			p->lottery.tickets = (tickets > 0) ? tickets : DEFAULT_TICKETS;
		}
	}
